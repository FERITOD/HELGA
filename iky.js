...
////< DAFTAR BUTTON BY Subs RIFQI BOTZ > ////
...
const prem3 = [{buttonId: `${prefix}buypremium`,buttonText: {displayText: `⬡ BUY PREMIUM `,},type: 1,},]
//////< FUNCTION LEVELING > ///////
...
ikyy.sendMessage(from, { "degreesLatitude": '', "degreesLongitude": '', "jpegThumbnail": fakeimage}, MessageType.location, {quoted: ikymekyy})
           break
   case 'kalender':
              reply(`
🐣Day ${week} ${weton} 
...
ikyy.sendMessage(from, { contentText: `${menu}`, footerText: `${wibu}`, buttons: [{ buttonId: `!command`, buttonText: { displayText: 'BACK TO MENU' }, type: 1 },{ buttonId: `!iklan`, buttonText: { displayText: '🏷️ SEWA BOT' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ikyads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
        ...
        case 'simi':
              ...
              buttonsMessage = {footerText:'Subs RIFQI BOTZ', imageMessage: imageMsg,
...
`
   sendButMessage(from, wibu, `Kenzy-Botz`, [
          {
...
`
   sendButMessage(from, wibu, `created Subs RIFQI BOTZ`, [
          {
            buttonId: `${prefix}menu`,
            buttonText: {
              displayText: `⬡ BACK TO MENU `,
...
▹ ${prefix}jadwalsholat
`
               sendButMessage(from, menu, `created Subs RIFQI BOTZ`, [
          {
            buttonId: `${prefix}menu`,
            buttonText: {
              displayText: `⬡ BACK TO MENU `,
   ...
   case 'command':
               list = []
               listmenu = [`groupmenu`,`photoxy`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`soundmenu`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`randomvideo`,`infomenu`,`othermenu`,`owner`,`gc`,`sewabot`]
               listmenuu = [`Menu Group`,`Photo Oky`,`Ephoto Menu`,`Random Image`,`Nsfw Anime`,`Movie&Story`,`Sound Menu`,`RandomText`,`Islam Menu`,`Wibu Menu`,`Sticker Menu`,`Owner Command`,`Game Menu`,`For Fun Menu`,`Downloader`,`Random Video`,`Info Menu`,`Menu Lainnya`,`OwnerBot`,`Group RIFQI BOTZ🇮🇩`,`Rent this Bot`]
               nombor = 1
               startnum = 0
...
*Gopay : Minta Owner*
*Qris : Minta Owner*`
ikyy.sendMessage(from, await getBuffer(gopeynya), image, {quoted: kyy, caption: teksnya })
break
...
ajgg = args.join(" ")
reply('wait....')
kyyi = `https://leyscoders-api.herokuapp.com/api/harta-tahta?text=${encodeURI(q)}&apikey=IkyOgiwara`
sendStickerUrl(from, kyyi, { quoted: freply})
break
   ...
   case 'tiger2':
     ...
     exec(`wget "${ini_buffer}" -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
      ...
      break
           case 'ttg':
           teks = args.join(' ')
kyy = encodeURIComponent(teks)
if (args.length < 2) return 
let ane = await getBuffer(`https://api.lolhuman.xyz/api/attp?apikey=PunyaIkyAds&text=${kyy}`)
...
//------------------< Owner >-------------------
      ...
      case 'broadcast2':
             ...
             bc = await ikyy.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             ikyy.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 TESTT BC 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
ikyy.sendMessage(_.jid, 
	    {"contentText": `*「 TEST BC BG 」*\n\n${body.slice(4)}`,
	    "footerText": 'TEST BC OM�',
	    "buttons": [
	    {"buttonId": `#command`,
...
if (!isGroup && isCmd && !kyy.key.fromMe){
	tek = `Subs RIFQI BOTZ`
kyyi = `Maaf @${senderr.split('@')[0]}, command ${prefix + command} tidak ada dalam menu`
sendButMessage (from, kyyi, tek, nomenu, 'extendedTextMessage', {sendEphemeral:true, quoted:kyy, mentionedJid:[senderr]})
}
	} 
